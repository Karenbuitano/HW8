...
// *****************************************************************************
// *****************************************************************************
int display_message_i(char message,int counter, int rownum);
/* Recieve data buffer */
...
void APP_Tasks (void )
{
    ...
    switch(appData.state)
    {
        ...
        case APP_STATE_MAIN_TASK:
          ...
          int tt; 
         for (tt=0; tt<62;tt++){
             writing[tt]=receiveDataBuffer[(tt+2)];
          }
          
          int rownum;
         sprintf(message, "%s", writing);
          //sprintf(message, "karen");
           //rownum= 6;
           rownum = receiveDataBuffer[1]; 
           int counter=0;
           while(message[counter]) {
     display_message_i(message[counter],counter,rownum);
    counter++;
           }
   display_draw();
                        ...
                        if(appData.hidDataTransmitted)
                        {
                           ...
                           short accels[3];
                            ...
                            if (_CP0_GET_COUNT()<200000) {
                                for (ll=0;ll>6;ll++) {
                                    appData.transmitDataBuffer[ll+1] = 0x00;
                                }
                            }
                            else {
                                ...
                                acc_read_register(OUT_X_L_A, (unsigned char *) accels,6);
                                //appData.transmitDataBuffer[1] = 0x10;
                                //appData.transmitDataBuffer[1] = accels[0]>>8;       // high bit of x acceleration
                                //appData.transmitDataBuffer[2] = accels[0]&0xFF;     // low bit of x acceleration
                                //appData.transmitDataBuffer[3] = accels[1]>>8;       // high bit of y acceleration
                                //appData.transmitDataBuffer[4] = accels[1]&0xFF;     // low bit of y acceleration
                                appData.transmitDataBuffer[1] = accels[2]>>8;       // high bit of z acceleration
                                appData.transmitDataBuffer[2] = accels[2]&0xFF;     // low bit of z acceleration
                            }
...
int display_message_i(char message,int counter, int rownum) {
        ...
        int ii, k, dummy[8][1] = {1, 1, 1, 1, 1, 1, 1, 1}, line, bits[8][5];
        ...
        for (col=0; col<5; col++) {
            for (row=0; row<8; row++) {
                ...
                display_pixel_set(row+rownum, col+(counter*5),bits[row][col]);
            }
        }
}
